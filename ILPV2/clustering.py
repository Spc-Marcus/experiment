...
# Add import for max_one models
try:
    from ..max_one import max_Ones_gurobi, max_Ones_comp_gurobi
except ImportError:
    try:
        sys.path.insert(0, str(Path(__file__).parent.parent))
        from max_one import max_Ones_gurobi, max_Ones_comp_gurobi
    except ImportError:
        print("Warning: max_one models not available")
        max_Ones_gurobi = None
        max_Ones_comp_gurobi = None
...
def clustering_full_matrix(
        ...
        solver_type: str = "gurobi"  # NEW: Add solver selection
    ...
    solver_type : str, optional
        Solver to use: "gurobi" (default), "max_ones", or "max_ones_comp"
    ...
    if len(regions) > 0:
        for idx, region in enumerate(regions):   
            ...
            if len(remain_cols) >= min_col_quality:
                logger.debug(f"Processing region {idx} with {len(remain_cols)} columns using {solver_type} solver")
    ...
    logger.debug(f"Clustering completed with {solver_type}: {len(steps_result)} total steps found")
    ...
    logger.info(f"Found {len(valid_steps)} valid clustering steps using {solver_type}")
...
def clustering_step_with_stats(input_matrix: np.ndarray,
    ...
    logger.debug(f"Starting clustering step on {matrix1.shape[0]} rows, {matrix1.shape[1]} columns using {solver_type}")
    ...
    while len(remain_rows) >= min_row_quality and len(current_cols) >= min_col_quality and status:
        iteration += 1
        logger.debug(f"Clustering iteration {iteration}: {len(remain_rows)} rows, {len(current_cols)} columns, pattern={'positive' if clustering_1 else 'negative'}, solver={solver_type}")
        
        # Apply quasi-biclique detection on appropriate matrix with selected solver
        if clustering_1:
            # Search for positive patterns (dense regions of 1s)
            rw, cl, status = find_quasi_biclique(matrix1[remain_rows][:, current_cols], error_rate, solver_type)
        else:
            # Search for negative patterns (dense regions of 0s in original)
            rw, cl, status = find_quasi_biclique(matrix0[remain_rows][:, current_cols], error_rate, solver_type)
    ...
    logger.debug(f"Clustering step completed with {solver_type}: {len(rw1)} positive reads, {len(rw0)} negative reads, {len(current_cols)} columns")
...
def find_quasi_biclique(
    ...
    """
    Find a quasi-biclique using the specified solver.
    ...
    input_matrix : np.ndarray
        Input binary matrix
    error_rate : float, optional
        Maximum fraction of 0s allowed in the quasi-biclique
    solver_type : str, optional
        Solver to use: "gurobi", "max_ones", or "max_ones_comp"
    """
    
    if solver_type == "max_ones":
        return find_quasi_biclique_max_ones(input_matrix, error_rate)
    elif solver_type == "max_ones_comp":
        return find_quasi_biclique_max_ones_comp(input_matrix, error_rate)
    else:
        # Default to original Gurobi implementation
        return find_quasi_biclique_original(input_matrix, error_rate)

def find_quasi_biclique_original(
    input_matrix: np.ndarray,
    error_rate: float = 0.025
) -> Tuple[List[int], List[int], bool]:
    """Original Gurobi implementation."""
    ...
    logger.debug(f"Starting original Gurobi quasi-biclique detection on {m}x{n} matrix")
...
def find_quasi_biclique_max_ones(
    input_matrix: np.ndarray,
    error_rate: float = 0.025
) -> Tuple[List[int], List[int], bool]:
    """Find quasi-biclique using max_Ones model."""
    
    if max_Ones_gurobi is None:
        logger.warning("max_Ones_gurobi not available, falling back to original solver")
        return find_quasi_biclique_original(input_matrix, error_rate)
    ...
    logger.debug(f"Starting max_Ones quasi-biclique detection on {m}x{n} matrix")
    
    try:
        # Prepare data for max_Ones model
        rows_data = [(i, int(np.sum(X_problem[i, :]))) for i in range(m)]
        cols_data = [(j, int(np.sum(X_problem[:, j]))) for j in range(n)]
        edges = [(i, j) for i in range(m) for j in range(n) if X_problem[i, j] == 1]
        ...
        if model.Status == grb.GRB.OPTIMAL:
            ...
            logger.debug(f"max_Ones found solution: {len(selected_rows)} rows, {len(selected_cols)} columns")
            return selected_rows, selected_cols, True
        else:
            logger.debug(f"max_Ones optimization failed with status: {model.Status}")
            return [], [], False
            
    except Exception as e:
        logger.warning(f"max_Ones solver failed: {e}, falling back to original")
        return find_quasi_biclique_original(input_matrix, error_rate)
...
def find_quasi_biclique_max_ones_comp(
    ...
    """Find quasi-biclique using max_Ones_comp model."""
    
    if max_Ones_comp_gurobi is None:
        logger.warning("max_Ones_comp_gurobi not available, falling back to original solver")
        return find_quasi_biclique_original(input_matrix, error_rate)
    ...
    logger.debug(f"Starting max_Ones_comp quasi-biclique detection on {m}x{n} matrix")
    
    try:
        # Prepare data for max_Ones_comp model
        rows_data = [(i, int(np.sum(X_problem[i, :]))) for i in range(m)]
        cols_data = [(j, int(np.sum(X_problem[:, j]))) for j in range(n)]
        edges = [(i, j) for i in range(m) for j in range(n) if X_problem[i, j] == 1]
        ...
        if model.Status == grb.GRB.OPTIMAL:
            ...
            logger.debug(f"max_Ones_comp found solution: {len(selected_rows)} rows, {len(selected_cols)} columns")
            return selected_rows, selected_cols, True
        else:
            logger.debug(f"max_Ones_comp optimization failed with status: {model.Status}")
            return [], [], False
            
    except Exception as e:
        logger.warning(f"max_Ones_comp solver failed: {e}, falling back to original")
        return find_quasi_biclique_original(input_matrix, error_rate)
