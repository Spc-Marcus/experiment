...
def find_quasi_biclique(
    ...
    **Constraints**:
    ...
    logger.debug(f"Using full matrix: {m} rows x {n} columns")
    ...
    try:
        with suppress_gurobi_output():
            ...
            model.Params.TimeLimit = 300  # 5 minutes = 300 seconds
    except Exception as e:
        logger.warning(f"Failed to initialize Gurobi environment: {e}")
        return [], [], False

    # Create optimization model with full matrix
    # Decision variables for all rows, columns, and cells
    lpRows = model.addVars(rows_sorted, lb=0, ub=1, vtype=grb.GRB.INTEGER, name='rw')
    lpCols = model.addVars(cols_sorted, lb=0, ub=1, vtype=grb.GRB.INTEGER, name='cl')
    lpCells = model.addVars([(r, c) for r in rows_sorted for c in cols_sorted], 
