...
def process_all_matrices(base_dir: str = None) -> List[Dict[str, Any]]:
    ...
    # Filter files by haplotype count
    suitable_files = []
    for csv_file in csv_files:
        haplotype_count = extract_haplotype_count(str(csv_file))
        if haplotype_count >= 4:
            suitable_files.append(csv_file)
            print(f"Including: {csv_file.name} (haplotypes: {haplotype_count})")
        else:
            print(f"Skipping: {csv_file.name} (haplotypes: {haplotype_count} < 4)")
    
    print(f"Processing {len(suitable_files)} files with haplotype count >= 4")
    
    if len(suitable_files) == 0:
        print("No files found with haplotype count >= 4")
        return results
    
    for i, csv_file in enumerate(suitable_files):
        try:
            print(f"Processing {i+1}/{len(suitable_files)}: {csv_file}")
...
def save_results_to_csv(results: List[Dict[str, Any]], output_file: str = "experiment_results.csv"):
    ...
    print("\n=== EXPERIMENT SUMMARY (Haplotypes >= 4) ===")
...
def run_experiments():
    """Main function to run all experiments (haplotypes >= 4 only)."""
    ...
    output_file = f"experiment_results_hap4plus_{timestamp}.csv"
